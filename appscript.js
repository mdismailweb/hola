...
/**
 * Delete a shift entry from the RealTimeShifts sheet
...
function deleteShift(dataOrId) {
  try {
    ...
    Logger.log(`🗑️ Attempting to delete shift: ${shiftId}`);
    ...
    if (lastRow <= 1) {
      return { success: false, message: 'No shifts found in the sheet' };
    }

    // Get all shift IDs from column A (Shift ID column)
    const shiftIds = sheet.getRange(2, 1, lastRow - 1, 1).getValues();

    // Find the row with matching shift ID
    for (let i = 0; i < shiftIds.length; i++) {
      const currentShiftId = String(shiftIds[i][0]).trim();

      if (currentShiftId === String(shiftId).trim()) {
        const rowNumber = i + 2; // +2 because array is 0-indexed and we start from row 2

        Logger.log(`✅ Found shift ${shiftId} at row ${rowNumber}`);

        // Delete the row
        sheet.deleteRow(rowNumber);

        Logger.log(`🗑️ Successfully deleted shift ${shiftId} from row ${rowNumber}`);

        return {
          success: true,
          message: `Shift ${shiftId} deleted successfully`,
          data: { shiftId, deletedRow: rowNumber }
        };
      }
    }

    // Shift ID not found
    Logger.log(`❌ Shift ${shiftId} not found in sheet`);
    return {
      success: false,
      message: `Shift with ID ${shiftId} not found`
    };
